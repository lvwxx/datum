...
**整体气质:简约现代风** —— 干净、留白充足、克制配色、无多余装饰。

- **双主题,一键切换**:
  - **亮色**(清爽亮色):白底、浅灰分区线、蓝色强调色,信息密度高、白天通透
  - **暗色**(macOS 原生深灰):柔和深灰底、低对比、蓝色强调色,与亮色气质统一、切换无割裂感
- **字体**:正文/界面用系统字体(SF / `-apple-system`);SQL 编辑器与结果表格用等宽字体
- **强调色**:蓝色(选中、可点击、运行按钮)
- **环境角标色**:`PROD` 红(醒目防误操作)/ `STAGING` 橙黄 / `LOCAL` 青
- **脏单元格高亮**:暂存未提交的改动用浅蓝/强调色底标记,提交后还原
