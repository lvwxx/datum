...
### 实现说明:PG 编辑的 SQL 构造与安全

`pg_commit_edits` 生成 `UPDATE` 时,**值(new_value、pk_value)以单引号转义(`'`→`''`)后内联为字符串字面量,标识符(表/列/主键列)以双引号转义(`"`→`""`)**,而非全部用绑定参数。原因:单元格值从前端以文本传来,需要让 PostgreSQL 隐式强制转换到任意标量列类型(int/bool/timestamp 等);若用绑定参数,tokio-postgres 会按列类型尝试把 String 序列化成 int8 等而失败。单引号转义防止了通过值注入 SQL;这是自用工具(连自己的库)可接受的权衡。NULL 与数组/JSON 等复杂类型暂不支持结构化编辑(已知限制)。
